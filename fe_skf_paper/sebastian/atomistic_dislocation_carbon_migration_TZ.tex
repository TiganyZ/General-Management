...
\section{Results}
\label{sec:org0c5912c}

\subsection{Peierls Potential}
\label{sec:org12e8c6a}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{Images/hard-split_transition.png}
\caption{Peierls potential along the hard-split line. One can see in s-d tight-binding model is similar to the EAM potential: it decreases constantly from the hard core to the split core, which will result in a kink shape which goes through the split core position. In DFT one finds a saddle point between the hard core and the midpoint, in which a kink in the transitional state will go through. \label{hardsplittransition}}
\end{figure}



The expected transition kink-shape will differ compared to that of DFT, with dislocation core
positions possibly being situated closer to/at the split core position, similar to EAM potentials
\cite{Itakura2012}. We can see this by following the Peierls potential along the H-S direction,
(see figure \ref{hardsplittransition}). We see that the Itakura potential has a saddle point, along this
direction, whereas in the tight-binding case there is no saddle point: the Peierls potential
decreases monotonically from the hard core to the split core position. However, calculation of the
Peierls barrier between two easy core positions in the canonical \(d\text{-band}\) version of this
tight-binding model (where \(s\text{-}d\) hybridisation and non-orthogonality are simply turned
off), found the transition state to go through the metastable point, as in DFT
\cite{Simpson2019}. The inclusion of s-d hybridisation in the tight-binding model seemingly reduces
the repulsive interaction between the iron atoms, as can be seen by the favouring of the split
core in our calculations, whereas in the canonical d-band model, there is sufficient repulsion to
recover the saddle point in the Peierls potential between the cores. To verify this, a
recalculation of the Peierls potential using the canonical d-band model is necessary. Further
Line-tension/NEB calculations are necessary to verify the dislocation line shape and discrepancies
in the kink-pair formation enthalpy.

\subsection{Preliminary calculations}
\label{sec:orga5e7068}
