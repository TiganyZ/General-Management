...
C ... e, de and dde are the values and derivatives of the polynomial in the region r1 , r < rc
     ...
     - So the form of the polynomial used is
       ...
       - \[  b = frac{1}{(r_1-r_2)^4} \big{ f'(r_1)*(r_1-r_2) - 3*f(r_1) \big}  \]
       - \[ \frac{1}{(r_1 - r_2)^5} x \]
       - \[  c = \frac{ f(r_1) }{ (r_1-r_2)^3} \]
       - Where $f(x)$ is the function that needs to be cut off.
     - 
...
**** Velocity Verlet Algorithm
     ...
     - \[ \mathbf{R}_{I}(t + \delta t) = \mathbf{R}_{I}(t) +  \mathbf{\dot{R}}_{I}\delta t + \frac{f_I(t)}{M_{I}}\delta t^3
       \mathcal{O}(\delta t^{3})  \]
     - \[ \mathbf{ \dot{R} }_{I}(t + \delta t) = \mathbf{\dot{R}}_{I}(t) +
       \frac{1}{2 M_{I}} [ \mathbf{f}_{I}(t + \delta t) + \mathbf{R}_{I}(t)
       ] + \mathcal{O}(\delta t^{3})  \]

**** How to calculate the forces
     - Use the Hellmann-Feynman theorem.
       - $$ \mathbf{f}_{I} = < \psi^{0}_{\mathbf{R}} | \frac{\partial}{\partial
         \mathbf{R}_{I}}\hat{H} |\psi^{0}_{\mathbf{R}} >   $$
