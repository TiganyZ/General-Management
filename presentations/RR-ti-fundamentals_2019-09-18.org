...
- Parameter set for TB model optimised by two evolutionary algorithms: genetic
  and particle swarm.
- More optimisation necessary. Interim model used for simulations.
- Similar tests can improve the optimisation procedure such
...
|-----------------------------------+-------+-------+--------|
| $a_{\alpha}$              [bohr]  |  5.56 |  5.20 |   5.57 |
| $c/a_{\alpha}$            [bohr]  |  1.59 |  9.15 |   1.58 |
| $C_{11}$                  [GPa ]  | 180.7 | 218.1 |  176.1 |
| $C_{33}$                  [GPa ]  | 206.1 | 263.4 |  190.5 |
| $C_{44}$                  [GPa ]  |  48.3 |  -8.5 |   50.8 |
| $C_{12}$                  [GPa ]  |  95.2 |  93.6 |   86.9 |
| $C_{13}$                  [GPa ]  |  66.5 |  54.4 |   68.3 |
| $a_{\omega}$              [bohr]  |  8.84 |  8.34 |   8.73 |
| $c_{\omega}$              [bohr]  |  5.38 |  5.30 |   5.32 |
| $a_{\beta}$               [bohr]  |  6.26 |    -- |   6.17 |
| $\Delta E(\omega-\alpha)$ [mRyd ] | -1.10 | 14.34 |  -0.73 |
...
#+ATTR_HTML: :width 70% :style position:relative;top:0px;left:-350px;z_index:1;
[[file:~/Documents/docs/Management/Images/gibbs_free-energies_per_atom_2019-09-12.png]]
...
|-----------+----------------------+-----+--------------------+--------------|
| Basal     | $I_{1}$ (AB\vert CB) | 198 | 148 $^{[1]}$       | --           |
|           | $I_{2}$ (AB\vert CA) | 242 | 260 $^{[1]}$       | 290 $^{[2]}$ |
|-----------+----------------------+-----+--------------------+--------------|
...
* Defect Clusters 
- Vacancies and solutes can form complexes which lower their energy (defactant
  model).
- This can cause more defects in material at given temperature than otherwise expected.
- More defects can result in increased number of precipitates due to more nucleation sites.
- This defect mediated mechanism may be behind Felicity's results of increased
  $\alpha_2$ precipitation. 



** Calculation Details
- Först$^{[6]}$ calculated energetics of defect complexes with associated local
  force-constant matrix.
- Alloys need to be treated as non-equilibrium.
...
** Ti_3Al
- Both Al and Ti vacancies. 
- Affinity varies. 



* Future Work 
- Finish Ti$_3$Al defect cluster calculations in DFT. 
- Maybe extend Ti with Al distributed randomly. 
- Validate Ti defect cluster results in DFT
