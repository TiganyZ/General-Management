...
** Necessary Calculations
    ...
    1) Kink-pair formation enthalpy *with* and *without*
       ...
       - Kink-nucleation enthalpy with hydrogen $E_{KH}$ and without
         ...
         the $[111](1\bar{1}0)$ direction using *string method*.
...
** Line tension Model 
   1) Interaction energy between a hydrogen interstitial and a screw dislocation.
...
** KMC
   ...
   - TST expresses the rates of kink-pair nucleation and kink migration events in terms of their migration barriers. 

   - TST matrix provides a physically consistent description of the energetics and thermally activated formation of a double-kink:

     \[ J_{kp} (\sigma_{\text{eff}}) = f_{kp} \exp{ \Big(  -\frac{\Delta E_{kp} (\sigma_{\text{eff}})}{k_B T} \Big) } \]

   - $\Delta E_{kp}(\sigma_{\text{eff}})$ is the stress dependent activation energy of kink-pair formation.
   - $f_{kp}$ is the pre-exponential "frequency" factor.
   - Local resolved stress is $\sigma_{\text{eff}} = \sigma_{\text{self}} + \sigma_{\text{solute}}$
   - $\sigma_{\text{self}}$ is the stress exerted on a dislocation segment by all other dislocation segments
     - This can be calculated from the gradient of the self energy due to virtual displacement  [Hirth and Lothe].
   ...
   - Kink-pair activation energy at various shear stresses, here the
     experimental one was used
       - With hydrogen $\Delta E_{kp},H$
       - Without hydrogen $\Delta E_{kp}$
       - (This can be done with/without line tension model, e.g. EAM or TB)
