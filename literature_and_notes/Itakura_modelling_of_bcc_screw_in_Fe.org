...
*** Line tension model 
    ...
    - This is expressed as an enthalpy of a curved screw dislocation configuration specified by the core positions $\mathbf{P}_j$ at each atomic layer $j$ of thickness $b$. 
      ...
      \[ E_{LT} = \frac{K}{2} \sum_j ( \mathbf{P}_j - \mathbf{P}_{j+1})^2 + \sum_j E_{\mathbf{P}_j}(\mathbf{P}_j) + \{ ( \sigma * \mathbf{b} ) \times \mathbf{l}\} \cdot \mathbf{P}_j - \sum_{j,k} E_H( | \mathbf{P}_j - \mathbf{P}_{j+1} | )   \]
