...
*** Line tension Model
    ...
    - This is a /plane wave expansion/ of the *Peierls Energy* from [[file:~/Documents/docs/Management/papers/birnbaum1994.pdf]]

    - Using this model a satisfactory shape of kinks is seen.
    - Kink pair energy using the MD gives 0.78 eV, line tension model
      gives 0.69 eV.
    - MD shows kink shape and energy is asymmetric between two types
      of kinks so there should be highre order terms of the gradient
      $( \mathbf{P}_j - \mathbf{P}_{j-1})$ in the energy
    - Errors of the line tension model can be attributed to it.
    - One can show that the kink formation energy is proportional to
      $\sqrt{KE_P}$,
      - So the uncertainty in $K$ only modestly affects the estimate
        of the kink formation energy.
...
*** Kink-pair formation energies w/wo Carbon
    
    - Wang /et al/ have calculated the kink-pair formation energies
      with and without carbon using NEB.
    - They also looked at the kink-pair migration energies with and
      without carbon. 
    - They put carbon in different sites around the core, and used
      NEB, with the middle image being a kink-pair created manually. 

****  Results
     
     - They have only looked at the actual energy barriers to form the
       kinks in different configurations in carbon.
     - They have *not* looked at the kink-pair formation energy at
       different stresses whatsoever.
   
**** What needs to be done
     
     - Calculation of the kink-pair formation energy at different
       stresses either using the EAM or using tight binding.
     
       

*** Velocity of screw segment
    
    - This has not been achieved yet. 


*** Solution energies of carbon around core

    - This has been done in DFT
    - It has also been done in the Domain-Becquart EAM model. 
    - It will have to be redone in the case of a tight binding model. 



*** Stress dependence of solution energies
    
    - This has not been done yet either. 


    
** Line tension Model 
   1) Interaction energy between a hydrogen interstitial and a screw
      dislocation. 
   2) Kink-pair formation energy /as a function of carbon concentration/
   3) Kink-pair migration energy /as a function of carbon concentration/
   
   This data is used to fit the line tension model. 
   Kink-pair formation enthalpy was obtained from this. 
...
** KMC
   ...
   - Kinetics completely determined by transition rates.
   - TST expresses the rates of kink-pair nucleation and kink
     migration events in terms of their migration barriers.  

   - TST matrix provides a physically consistent description of the
     energetics and thermally activated formation of a double-kink: 

     \[ J_{kp} (\sigma_{\text{eff}}) = f_{kp} \exp{ \Big(  -\frac{\Delta E_{kp} (\sigma_{\text{eff}})}{k_B T} \Big) } \]

   - $\Delta E_{kp}(\sigma_{\text{eff}})$ is the stress dependent
     activation energy of kink-pair formation. 
   - $f_{kp}$ is the pre-exponential "frequency" factor.
   - Local resolved stress is $\sigma_{\text{eff}} =
     \sigma_{\text{self}} + \sigma_{\text{solute}}$ 
   - $\sigma_{\text{self}}$ is the stress exerted on a dislocation
     segment by all other dislocation segments 
     - This can be calculated from the gradient of the self energy due
       to virtual displacement  [Hirth and Lothe]. 

   - Nucleation rates of stable kink-pairs. 
     
   - Kink-pair activation energy at various shear stresses, here the
     experimental one was used
       - With hydrogen $ΔE_{kp},H$
       - Without hydrogen $ΔE_{kp}$
       - (This can be done with/without line tension model, e.g. EAM
         or TB) 
       - /Maybe Experimentally determined/ kink-pair activation energies.
       - This was done in the case of hydrogen due to the fact there
         are differences between the experimentally determined
         enthalpy and the predicted one from DFT.
       - Differences that may contribute are the zero-point energy and
